...
BOOL CALLBACK LocaleEnumProc(LPWSTR localeName, DWORD dwFlags, LPARAM lparam) {
    ...
    fwprintf(fp, L"%ls,0x%04lX,%ls,%ls,%ls,%ls,%ls,%ls,%ls,%ls\n",
...
int main() {
    ...
    fwprintf(fp, L"Locale Name,LCID,ANSI CodePage,ANSI Character Set,OEM CodePage,OEM Character Set,Native Language Name,English Language Name,Country,Script\n");
    if (ferror(fp)) {
